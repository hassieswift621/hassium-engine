...
namespace Hassie.HassiumEngine.Input
{
    ...
    /// </summary>
    public class RightMouseEventHandler : AbstractEventHandler<RightMouseEvent>
    {
        private RightMouseEvent state; // The current state.

        public override void Subscribe(EventHandler<RightMouseEvent> eventHandler)
        {
            // Call base subscribe.
        ...
        public override void Unsubscribe(EventHandler<RightMouseEvent> eventHandler)
        {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (state == null && mouseState.RightButton == ButtonState.Pressed)
            {
                // Store state.
                state = new RightMouseEvent(mouseState.Position.ToVector2(), ButtonState.Pressed);
            ...
            else
            {
                ...
                if (state.State == ButtonState.Released && mouseState.RightButton == ButtonState.Pressed)
                {
                    // Store state.
                    state = new RightMouseEvent(mouseState.Position.ToVector2(), ButtonState.Pressed);
                ...
                else if (state.State == ButtonState.Pressed && mouseState.RightButton == ButtonState.Released)
                {
                    // Store state.
                    state = new RightMouseEvent(null, ButtonState.Released);
